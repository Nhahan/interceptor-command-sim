...
7. result is reviewed through post-engagement review / replay

## Required Properties

- shows command -> validation -> state transition -> assessment
- shows multi-client visibility
- can generate meaningful post-engagement review output
...
The runtime auto-archives on the tick after assessment, so the normal GUI flow is `Start -> Track -> Ready -> Engage -> Review -> Reset`.
...
- post-archive control requests remain rejected while post-engagement review stays available
...
- where the post-engagement review cursor is in replay mode
