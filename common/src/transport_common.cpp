...
icss::protocol::SnapshotPayload to_snapshot_payload(const icss::core::Snapshot& snapshot) {
    return {
        ...
        icss::core::to_string(snapshot.interceptor_status),
        icss::core::to_string(snapshot.engage_order_status),
        snapshot.assessment.ready,
        icss::core::to_string(snapshot.assessment.code),
...
icss::protocol::TelemetryPayload to_telemetry_payload(const icss::core::Snapshot& snapshot,
    ...
    return {
        snapshot.envelope,
        snapshot.telemetry,
        icss::core::to_string(snapshot.display_connection),
        event_tick,
        std::move(event_type),
