...
bool active_live_phase(icss::core::SessionPhase phase) {
    return phase == icss::core::SessionPhase::Detecting
        ...
        || phase == icss::core::SessionPhase::AssetReady
        || phase == icss::core::SessionPhase::CommandIssued
        || phase == icss::core::SessionPhase::Engaging;
}
...
bool asset_motion_visual_visible(const ViewerState& state) {
    return state.snapshot.asset.active
        ...
        && recent_motion_in_history(state.asset_history)
        && speed_sq(state.snapshot.asset_velocity) > 0.01F;
}
...
bool predicted_marker_visual_visible(const ViewerState& state) {
    return state.snapshot.phase == icss::core::SessionPhase::Engaging
        && state.snapshot.command_status == icss::core::CommandLifecycle::Executing
        && state.snapshot.predicted_intercept_valid;
}
