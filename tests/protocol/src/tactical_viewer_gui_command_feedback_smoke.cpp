...
ChildProcess spawn_gui_viewer(const std::filesystem::path& dump_path,
    ...
    if (pid == 0) {
        ...
        std::vector<std::string> argv_storage {
            ...
            "--headless",
            "--hidden",
            "--auto-controls", "Start,Track,Ready,Engage",
            "--duration-ms", "1400",
...
int main() {
    ...
    assert(icss::testsupport::minijson::require_field(object, "last_control_message").as_string().find("engage order accepted") != std::string::npos);
    ...
    const auto interceptor_status = icss::testsupport::minijson::require_field(object, "interceptor_status").as_string();
    assert(interceptor_status == "intercepting" || interceptor_status == "complete");
    assert(icss::testsupport::minijson::require_field(object, "command_visual_active").as_bool() || engage_order_status == "completed");
    ...
    const auto asset_motion_visible = icss::testsupport::minijson::require_field(object, "interceptor_motion_visual_visible").as_bool();
    ...
    if (phase == "intercepting" && engage_order_status == "executing") {
    ...
    if (icss::testsupport::minijson::require_field(object, "assessment_code").as_string() == "intercept_success") {
        assert(!target_active);
    }
