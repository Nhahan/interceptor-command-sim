...
int main() {
    ...
    send_json_frame(tcp_client.fd, "session_join", serialize(SessionJoinPayload{{1001U, 101U, 1U}, "fire_control_console"}));
    ...
    send_json_frame(tcp_client.fd, "track_acquire", serialize(TrackAcquirePayload{{1001U, 101U, 3U}, "target-alpha"}));
    assert(parse_command_ack(wait_for_json_frame(tcp_client.fd).payload).accepted);

    send_json_frame(tcp_client.fd, "interceptor_ready", serialize(InterceptorReadyPayload{{1001U, 101U, 4U}, "interceptor-alpha"}));
    assert(parse_command_ack(wait_for_json_frame(tcp_client.fd).payload).accepted);

    send_json_frame(tcp_client.fd, "engage_order", serialize(EngageOrderPayload{{1001U, 101U, 5U}, "interceptor-alpha", "target-alpha"}));
    assert(parse_command_ack(wait_for_json_frame(tcp_client.fd).payload).accepted);
